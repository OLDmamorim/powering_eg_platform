...
- Responde APENAS com o JSON, sem texto adicional`;
  ...
  dashboardVolante: router({
    ...
    // Registar serviços do volante
    registarServicos: publicProcedure
      .input(z.object({
        token: z.string(),
        data: z.string(), // YYYY-MM-DD
        lojaId: z.number(),
        substituicaoLigeiro: z.number().min(0),
        reparacao: z.number().min(0),
        calibragem: z.number().min(0),
    ...
    getHistoricoServicosLoja: publicProcedure
      .input(z.object({
        token: z.string(),
        lojaId: z.number(),
      }))
      .query(async ({ input }) => {
        ...
        const historico = await db.getServicosVolantePorLoja(tokenData.volante.id, input.lojaId, 30);
        return historico;
      }),

    // Obter totais de serviços por período
    getTotaisServicos: publicProcedure
      .input(z.object({
        token: z.string(),
        dataInicio: z.string(),
        dataFim: z.string(),
