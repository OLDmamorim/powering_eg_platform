...
export async function getAllTodos(filtros?: {
  ...
  return result.map(todo => {
    ...
    if (userContext) {
      ...
      // 2. Criado por uma das minhas lojas E NÃO é interna (comunicação loja->gestor)
      // NOTA: Tarefas internas (isInterna=true) criadas pela loja para si própria NÃO aparecem para o gestor
      if (todo.atribuidoUserId === userContext.userId) {
        ...
        // Tarefa criada por uma das minhas lojas - só é para mim se NÃO for interna
        if (!todo.isInterna) {
          paraMim = true;
        }
      }
...
export async function pesquisarMatriculaPorPrefixo(
  prefixo: string,
  lojaId?: number,
  limit: number = 50
): Promise<Array<{
  ...
  // Normalizar: remover espaços e hífens, uppercase
  const prefixoNorm = prefixo.toUpperCase().replace(/[\s\-]/g, '').trim();
  if (prefixoNorm.length < 2) return []; // Mínimo 2 caracteres
  
  // Construir condição WHERE - pesquisar matrícula removendo hífens
  const conditions = [
    sql`UPPER(REPLACE(${eurocodesFichas.matricula}, '-', '')) LIKE ${prefixoNorm + '%'}`
  ];
