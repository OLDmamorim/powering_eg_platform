...
async function test() {
  try {
    ...
    console.log('PDF gerado, tamanho base64:', pdf.length);
    const fs = await import('fs');
    fs.writeFileSync('/tmp/test-pdf-short.pdf', Buffer.from(pdf, 'base64'));
    console.log('PDF salvo em /tmp/test-pdf-short.pdf');
  } catch (err) {
